...
#define VERSION 22 //id for the validation
#define FILTER_TYPE 14// Village with desert temple
#define DEBUG 0
#include <string.h>
#include <time.h>
...
int templeLocation(int64_t lower48){
...
int valid_temple_not_biome(int64_t lower48){
  if (templeLocation(lower48) == 0){
    return -1001;
  }
  return 1;
}

int valid_village_and_desert_pyramid(int64_t lower48){
  int px,pz;
...
int valid_structures_and_types(int64_t lower48, int* fortressQuadrant, int filter_style){
  ...
  if (filter_style == 14){
    return valid_village_and_desert_pyramid(lower48);
  }
  return 1;
}
...
int templeBiome(int64_t seed, LayerStack* gp){
  const StructureConfig sconf = DESERT_PYRAMID_CONFIG;
  int valid;
  Pos p = getStructurePos(sconf, seed, 0, 0, &valid);
  int mc = MC_1_16;
...
int main () {
    ...
    printf("FSG for 1.14.4 by nasko222 \n");
